...
Create the project and Git repository.

Write the README outline before building so the requirements stay visible.

Ask your AI coding agent for a plan and review the files it proposes to create.

Create the Neon project, enable Managed Better Auth and the Data API, then create the contacts schema and RLS policies.

Build the smallest complete contact workflow from the UI through the database.

Add backend validation and an automated test.

Run the app locally and verify the two-user privacy test.

Push the repository to GitHub and deploy it on Vercel.

Configure production environment variables and run the full verification checklist on the live URL.

Finish the README with the live URL, screenshots, architecture, schema, tests, and deployment notes.
...
Import the repository into Vercel or deploy it with the Vercel CLI.

Add the production environment variables in Vercel.

Add the deployed Vercel domain to Neon Auth's trusted origins and confirm sign-in works there.

Open the public URL in a private browser window.

Create two accounts and repeat the privacy test in production.

Run every Definition of Done check against the deployed application.

Add the live URL and every grading artifact to the README.
